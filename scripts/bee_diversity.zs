...
<contenttweaker:bee_diversity>.addAdvancedTooltip(function(item) {   
  return "储存基因总数: §6" ~ D(item.tag).getInt("total");
});
<contenttweaker:bee_diversity>.addAdvancedTooltip(function(item) {   
  return "惩罚等级: §4" ~ D(item.tag).getInt("penalty");
});
<contenttweaker:bee_diversity>.addAdvancedTooltip(function(item) {
  return "时间奖励总计: §3" ~ toTime(D(item.tag).getInt("reward"));
});
<contenttweaker:bee_diversity>.addAdvancedTooltip(function(item) {
  return "§8上次获得的时间奖励: §7" ~ toTime(D(item.tag).getInt("last"));
});
